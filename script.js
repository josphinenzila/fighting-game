...
const updateGame = (p1, p2, gameState) => {
  ...
  if (p1.health <= 0 || p2.health <= 0) {
    gameState = true;
    resultDiv.innerText = newGame.declareWinner(gameState, p1, p2);
    return gameState;
  }
};
...
class Player {
  ...
  heal(player) {
    ...
    const hpAmount = Math.ceil(Math.random() * 10);
...
class Game {
  ...
  play(p1, p2) {
    ...
    return this.declareWinner(gameState, p1, p2);
    // Once isOver is TRUE run the declareWinner() method
  }
}

// ** Create 2 players using the player class **
let player1 = new Player("Josphine", 100, 10);
let player2 = new Player("Basil", 100, 7);
...
document.addEventListener("keydown", function (e) {
  ...
  if (e.key == "a" && p2.health > 0 && gameState == false) {
    p1.heal(p1);
...
document.addEventListener("keydown", function (e) {
  ...
  if (e.key == "l" && p1.health > 0 && gameState == false) {
    p2.heal(p2);
...
playButton.onclick = () => newGame.play(p1, p2);
